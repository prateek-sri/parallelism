...
{
    unsigned int i, bin_temp[16];
    for (i = 0; i < NUM_BINS; i++) bins[i] = 0;
    // Find the digit to be operated on starting from LSD
    for (i = 0; i < size; i++) bins[((arr[i] >> bit_cur) & 0xF)]++;
    for (i = 0; i < NUM_BINS; i++) 
        bin_temp[i] = bins[i];

    //printarray(bins,16);
    // Prefix sum on the digits
    prefix_sum(bin_temp, NUM_BINS);
    //printarray(bin_temp,16);

    // Sorted array in temp
    for (i = 0; i < size; i++) temp[bin_temp[(arr[i] >> bit_cur) & 0xF]++] = arr[i];
...
void radixsort(uint32_t* __restrict arr, uint32_t l)
{
...
#pragma omp parallel for
            ...
            {
                int k = i >> shf;
                int *temp_new = (int *) malloc(size * sizeof(int));
                prefix_sum(&bins[(k)*NUM_BINS], NUM_BINS);
                for(uint32_t j = i; j < ((i+size)>l?l:i+size); j++)
                {
                    temp_new[j-i] = (temp[j] >> bit) & 0xF;
                    uint32_t gindex = temp_bins[(k) + MAX_THREADS * temp_new[j-i]];
                    uint32_t lindex = bins[(k) * NUM_BINS + temp_new[j-i]];
                    //if (MODULO(bit, 8) == 0)
