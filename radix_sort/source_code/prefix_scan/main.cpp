...
double cudaScan_warp(int* start, int* end, int* resultarray);
double cudaScan_loopUnroll(int* start, int* end, int* resultarray);
...
int main(int argc, char** argv)
{
    ...
    int opt;
    static struct option long_options[] = {{"test", 1, 0, 'm'}, {"arraysize", 1, 0, 'n'}, {"input", 1, 0, 'i'},
    ...
    double cudaTime_warp = 50000.;
    double cudaTime_loopUnroll = 50000.;
    ...
    {  // test exclusive scan
        ...
        for (int i = 0; i < 3; i++)
        {
            if (useThrust)
                ...
                cudaTime_warp = std::min(cudaTime_warp, cudaScan_warp(inarray, inarray + N, resultarray));
            cudaTime_loopUnroll = std::min(cudaTime_loopUnroll, cudaScan_loopUnroll(inarray, inarray + N, resultarray));
        ...
        if (useThrust)
        {
            ...
            printf("Thrust Scan: %.3fx speedup\n", serialTime / cudaTime);
            printf("---------------------------------------------------------\n");
        }
        else
        {
            ...
            printf("CPU_time: %.3f ms\n", 1000.f * serialTime);
            printf("\n---------------------------------------------------------\n");
            printf("\nFully Optimised scan kernel\n", N);
            printf("\nInput size = %d\n", N);
            printf("GPU_time: %.3f ms\n", 1000.f * cudaTime);
            printf("Fully Optimised Scan: %.3fx speedup\n", serialTime / cudaTime);
            printf("---------------------------------------------------------\n");
            printf("\nWarp Optimised scan kernel\n", N);
            printf("\nInput size = %d\n", N);
            printf("GPU_time: %.3f ms\n", 1000.f * cudaTime_warp);
            printf("Warp optimized Scan: %.3fx speedup\n", serialTime / cudaTime_warp);
            printf("\n---------------------------------------------------------\n");
            printf("\nLoop Unrolled scan kernel without bank conflict handling\n", N);
            printf("\nInput size = %d\n", N);
            printf("GPU_time: %.3f ms\n", 1000.f * cudaTime_loopUnroll);
            printf("Without bank conflict resolution : %.3fx speedup\n", serialTime / cudaTime_loopUnroll);
            printf("\n---------------------------------------------------------\n");
        }
